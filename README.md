...
5. **배포 → 새 배포 → 웹 앱** 선택 후 배포
   - 권한: 본인 또는 조직 사용자
   - 액세스: 앱을 사용할 사용자 범위 선택

## 로컬 UI 미리보기
로컬에서 UI를 확인하려면 이 저장소 루트에서 다음을 실행하고 `index.html`로 접속하세요.

```bash
python -m http.server 8000
```

브라우저에서 `http://localhost:8000/index.html`로 접속하면 됩니다. `Not Found`가 뜨면 현재 폴더가 이 저장소 루트인지 확인해 주세요.
